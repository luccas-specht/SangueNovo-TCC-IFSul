...
export class CreateAllTables1622132797718 implements MigrationInterface {
  name = 'CreateAllTables1622132797718';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "tb_donation_donationstatus_enum" AS ENUM('Solicitado', 'Ativo', 'Finalizado')`
    );
    await queryRunner.query(
      `CREATE TABLE "tb_donation" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "appointment_date" TIMESTAMP WITH TIME ZONE NOT NULL, "donationStatus" "tb_donation_donationstatus_enum" NOT NULL DEFAULT 'Solicitado', "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "donatorId" uuid, "campaignId" uuid, CONSTRAINT "PK_8394e1eb4761c2442917a4a876a" PRIMARY KEY ("id"))`
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "tb_campaign_campaignstatus_enum" AS ENUM('Ativo', 'Solicitado', 'Finalizado', 'Recusado')`
    );
    await queryRunner.query(
      `CREATE TYPE "tb_campaign_priority_enum" AS ENUM('Alta', 'Media', 'Baixa')`
    );
    await queryRunner.query(
